...
def powell(F,x,h=0.1,tol=1.0e-6):
    ...
    df = numpy.zeros((n),dtype=float)   # Decreases of F stored here
    u =  numpy.identity(n)*1.0            # Vectors v stored here by rows
    for j in range(30):            # Allow for 30 cycles:
      ...
      # Check for convergence
        if math.sqrt(numpy.dot(x-xOld,x-xOld)/n) < tol: return x,j+1
      # Identify biggest decrease & update search directions
        iMax = int(numpy.argmax(df))
