import solar, math, supportingfunctions
...
class Shading:
  def __init__(self, digitalElevationModel,latitudeFloatingPoint,longitudeFloatingPoint,reduceRunTime):
  ...
  def update(self,timeDatetimeFormat):
    ...
    return fractionSolarBeamReceivedWithShading, fractionSolarBeamReceivedFlatSurface
  ...
  def report(self,sample,timestep):
    report(self.fractionSolarBeamReceived,generateNameST('Mfs', sample, timestep))
    report(self.solarCritAngle,generateNameST('Msc', sample, timestep))
    report(self.shaded,generateNameST('Msh', sample, timestep))
