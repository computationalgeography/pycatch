...
class SurfaceStore:
  def __init__(self, initialStore, maxStore, timeStepDuration):
  ...
  def report(self, sample, timestep):
    report(self.store,generateNameST('Ss', sample, timestep))
    report(self.amountToFlux(self.changeAmount),generateNameST('Sc', sample, timestep))
