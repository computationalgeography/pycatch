...
class EvapotranspirationPenman(component.Component):
  def __init__(self, timeStepDuration,albedo,maxStomatalConductance, vegetationHeight, LAI, \
    ...
    self.aboveVegHeight=scalar(0)
    self.aerodynamicRes=scalar(0)
    self.airTemperature=scalar(0)
    self.windVelocityAboveVegHeight=scalar(0)
  ...
  def potentialEvapotranspiration(self, \
    ...
    self.windVelocityNotZero=max(0.05,scalar(windVelocity)) #N FAO recommends u>0.5m/s
    ...
    netRadiation=max(0.00001,self.incomingShortwaveRadiation*(1.0-self.albedo)-netLongRadiation)
    ...
    self.windVelocityAboveVegHeight=self.windVelocityNotZero+(0.3/0.41)*ln(self.aboveVegHeight/observedWindHeight)

            
    #  aerodynamic resistance, Grace (1981)
    self.aerodynamicRes=ln((self.aboveVegHeight-0.7*self.vegHeight)/(0.1*self.vegHeight))**2/(0.41**2.0*self.windVelocityAboveVegHeight)   
    #self.aerodynamicRes=5.0
    #report(scalar(self.aerodynamicRes),'ra.map')
...
## inputs for testing
airTemperature=scalar(23)
relativeHumidity=scalar(0.6)
incomingShortwaveRadiationAtSurface=scalar(300.0)
incomingShortwaveRadiationFlatSurface=scalar(300.0)
